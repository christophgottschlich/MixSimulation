...
class PoissonMix(object):
    ...
    def receive_message(self, message):
        ...
        SIMLOGGER.log('Step: ' + str(self.env.now) + ' - PoissonMix ' + str(self.address) + ': Receiving Message with id: ' + str(message.message_id))
    ...
    def send_message(self, message):
        ...
        if len(tmp_message.route) > 0:
            ...
            if tmp_message.recipient.startswith('U'):
                ...
                return None
        SIMLOGGER.log('Step: ' + str(self.env.now) + ' - PoissonMix ' + str(self.address) + ': Sending Message with id: ' + str(tmp_message.message_id) + ' to address: ' + address)
    ...
    def validate_message(self, message):
        ...
        if current_ms < message.valid_intervals[0][0] or current_ms > message.valid_intervals[0][1]:
            ...
            SIMLOGGER.log('Step: ' + str(self.env.now) + ' - PoissonMix ' + str(self.address) + ': REJECTING Message with id: ' + str(message.message_id) + '. Message arrived after ' + str(current_ms) + 'MS and is not in the specified time window ' + str(message.valid_intervals[0]))
            SIMLOGGER.action_to_csv(str(self.env.now), self.address, 'reject', str(message.message_id), 'None')

            return False
        SIMLOGGER.log('Step: ' + str(self.env.now) + ' - PoissonMix ' + str(self.address) + ': ACCEPTING Message with id: ' + str(message.message_id) + '. Message arrived after ' + str(current_ms) + 'MS and is in the specified time window ' + str(message.valid_intervals[0]))
